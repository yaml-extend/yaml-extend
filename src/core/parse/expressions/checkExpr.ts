...
export function isMapExpr(map: YAMLMap): {
  isExpr: true;
  expr: string;
  scalar: Scalar;
};
...
export function isMapExpr(map: YAMLMap): {
  ...
  if (!map.flow) return { isExpr: false, expr: "", scalar: undefined }; // make sure it's a flow syntax
...
export function isSeqExpr(seq: YAMLSeq): {
  isExpr: true;
  expr: string;
  scalar: Scalar;
};
...
export function isSeqExpr(seq: YAMLSeq): {
  ...
  if (!seq.flow) return { isExpr: false, expr: "", scalar: undefined }; // make sure it's a flow syntax
...
export function isStringExpr(str: string): {
  isExpr: boolean;
  expr: string;
} {
  const tStr = str.trim(); // trim string
