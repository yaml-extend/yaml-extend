...
function verifyFilename(dir, directives) {
  ...
  // verify filename
  const filename =
    (_a = dir.filename) === null || _a === void 0 ? void 0 : _a.value;
  if (!filename) {
...
function verifyLocal(dir, directives) {
  ...
  // make sure that alias is used only once
  if (
    directives.import.some((d) => {
      var _a;
      return (
        ((_a = d.alias) === null || _a === void 0 ? void 0 : _a.value) === alias
      );
    })
  ) {
    const error = new YAMLExprError(
  ...
  const type =
    (_b = dir.yamlType) === null || _b === void 0 ? void 0 : _b.value;
  if (
    type &&
    (typeof type !== "string" ||
...
function verifyTag(dir, directives) {
  ...
  const handle =
    (_a = dir.handle) === null || _a === void 0 ? void 0 : _a.value;
  if (!handle) {
    const error = new YAMLExprError(
      dir.pos,
      "",
  ...
  if (
    directives.tag.some((d) => {
      var _a;
      return (
        ((_a = d.handle) === null || _a === void 0 ? void 0 : _a.value) ===
        handle
      );
    })
  ) {
  ...
  const prefix =
    (_b = dir.prefix) === null || _b === void 0 ? void 0 : _b.value;
  if (!prefix) {
    const error = new YAMLExprError(
      dir.pos,
      "",
...
function verifyVersion(dir, directives) {
  ...
  const version =
    (_a = dir.version) === null || _a === void 0 ? void 0 : _a.value;
  if (!version) {
    const error = new YAMLExprError(
      dir.pos,
      "",
  ...
  try {
    ...
    if (type === "IMPORT") {
      ...
      for (const [k, t] of Object.entries(params))
        resolvedParams[k] =
          (_a = t.value) === null || _a === void 0 ? void 0 : _a.value;
      const resolvedPath =
        (pathTok === null || pathTok === void 0 ? void 0 : pathTok.text) &&
        mergePath(pathTok.text, tempState);
      return {
        type: "IMPORT",
        rawLine,
        linePos,
...
function getImport(tokens, alias, validCheck) {
  ...
  for (const tok of tokens) {
    if (!tok.valid && validCheck) continue;
    if (
      ((_a = tok.alias) === null || _a === void 0 ? void 0 : _a.value) === alias
    )
      return {
...
function getParam(tokens, alias, validCheck) {
  ...
  for (const tok of tokens) {
    ...
    if (
      ...
      return {
        defauleValue:
          (_b = tok.defValue) === null || _b === void 0 ? void 0 : _b.value,
        yamlType:
          (_c = tok.yamlType) === null || _c === void 0 ? void 0 : _c.value,
      };
  }
}
...
async function handleModuleCache(state, tempState) {
  ...
  tempState.filename =
    (_a = getFilename(moduleCache.directives.filename, true)) !== null &&
    _a !== void 0
      ? _a
      : "";
}
...
async function initModuleCache(state, tempState) {
  ...
  tempState.filename =
    (_a = getFilename(directives.filename, true)) !== null && _a !== void 0
      ? _a
      : "";
}
function handleAST(tempState) {
  // pass source and options to yaml lib
  const AST = yaml.parseDocument(tempState.source, tempState.options);
  // add errors
  ...
  for (const e of errors) {
    ...
    if (e instanceof yaml.YAMLWarning)
...
function setParseEntery(state, tempState, parseEntery) {
  ...
  const comParams = {
    ...((_a = tempState.options.universalParams) !== null && _a !== void 0
      ? _a
      : {}),
...
})(exports.KeyValueTokenType || (exports.KeyValueTokenType = {}));
...
function tokenizeKeyValue(input, argsTok, tempState, depth, tokenizeTextFunc) {
  ...
  while (true) {
    ...
    tokens.push(...toks);
    if (
      ((_a = tokens[tokens.length - 1]) === null || _a === void 0
        ? void 0
        : _a.type) === exports.KeyValueTokenType.EOF
    )
      break;
  }
  // resolve any value tokens using text tokenizer
  for (const t of tokens)
    if (t.type === exports.KeyValueTokenType.VALUE)
      t.valueTokens = tokenizeTextFunc(
        (_b = t.raw) !== null && _b !== void 0 ? _b : "",
        t,
        tempState,
        depth
      );
...
function nextArgsToken$1(state, tempState, parentTok) {
  ...
  if (eof(state)) {
    ...
    eofToken = {
      type: exports.KeyValueTokenType.EOF,
      raw: "",
      text: "",
      value: "",
      quoted: false,
  ...
  if (ch === "=") {
    ...
    if (readValue) {
      ...
      linePos = getLinePosFromPos(tempState.lineStarts, pos);
      equalToken = {
        type: exports.KeyValueTokenType.EQUAL,
        raw: readValue.raw,
        text: readValue.text,
        value,
        quoted: false,
...
function readQuoted(state, tempState, parentTok) {
  ...
  const linePos = getLinePosFromPos(tempState.lineStarts, pos);
  const tok = {
    type: state.afterEqual
      ? exports.KeyValueTokenType.VALUE
      : exports.KeyValueTokenType.KEY,
    raw: readValue.raw,
    text: readValue.text,
    value,
    quoted: true,
...
function tokenizeArgs(input, exprTok, tempState, depth, tokenizeTextFunc) {
  ...
  while (true) {
    ...
    tokens.push(...toks);
    if (tokens[tokens.length - 1].type === exports.ArgsTokenType.EOF) break;
  }
  // resolve any value tokens using text tokenizer
  for (const t of tokens)
    if (t.type === exports.ArgsTokenType.KEY_VALUE)
      t.keyValueTokens = tokenizeKeyValue(
        (_a = t.raw) !== null && _a !== void 0 ? _a : "",
        t,
        tempState,
        depth,
        tokenizeTextFunc
...
function nextArgsToken(state, tempState, parentTok) {
  ...
  if (eof(state)) {
    ...
    linePos = getLinePosFromPos(tempState.lineStarts, pos);
    eofToken = {
      type: exports.ArgsTokenType.EOF,
      raw: "",
      text: "",
      value: "",
      quoted: false,
  ...
  if (ch === ",") {
    ...
    linePos = getLinePosFromPos(tempState.lineStarts, pos);
    commaToken = {
      type: exports.ArgsTokenType.COMMA,
      raw: readValue.raw,
      text: readValue.text,
      value,
      quoted: false,
  ...
  linePos = getLinePosFromPos(tempState.lineStarts, pos);
  keyValueToken = {
    type: exports.ArgsTokenType.KEY_VALUE,
    raw: readValue.raw,
    text: readValue.text,
    value,
    quoted: false,
...
function tokenizeExpr(input, textTok, tempState, depth, tokenizeTextFunc) {
  ...
  while (true) {
    ...
    tokens.push(...toks);
    if (tokens[tokens.length - 1].type === exports.ExprTokenType.EOF) break;
  }
  // tokenize args inside ARGS token
  for (const t of tokens)
    if (t.type === exports.ExprTokenType.ARGS)
...
function nextExprToken(state, tempState, parentTok) {
  ...
  if (eof(state)) {
    ...
    linePos = getLinePosFromPos(tempState.lineStarts, pos);
    eofToken = {
      type: exports.ExprTokenType.EOF,
      raw: "",
      text: "",
      value: "",
      quoted: false,
  ...
  if (ch === "." && !state.afterWhiteSpace) {
    ...
    if (readValue) {
      ...
      linePos = getLinePosFromPos(tempState.lineStarts, pos);
      dotToken = {
        type: exports.ExprTokenType.DOT,
        raw: readValue.raw,
        text: readValue.text,
        value,
        quoted: false,
  ...
  if (ch === "(" && !state.afterParen) {
    ...
    if (readValue) {
      ...
      linePos = getLinePosFromPos(tempState.lineStarts, pos);
      argsToken = {
        type: exports.ExprTokenType.ARGS,
        raw: readValue.raw,
        text: readValue.text,
        value,
        quoted: false,
  ...
  if (/\s/.test(ch)) {
    ...
    if (readValue) {
      ...
      linePos = getLinePosFromPos(tempState.lineStarts, pos);
      wsToken = {
        type: exports.ExprTokenType.WHITE_SPACE,
        raw: readValue.raw,
        text: readValue.text,
        value,
        quoted: false,
    ...
    if (readValue) {
      ...
      linePos = getLinePosFromPos(tempState.lineStarts, pos);
      typeToken = {
        type: exports.ExprTokenType.TYPE,
        raw: readValue.raw,
        text: readValue.text,
        value,
        quoted: false,
...
function readQuotedPath(state, tempState, parentTok) {
  ...
  const linePos = getLinePosFromPos(tempState.lineStarts, pos);
  const tok = {
    type: state.baseDefined
      ? exports.ExprTokenType.PATH
      : exports.ExprTokenType.BASE,
    raw: readValue.raw,
    text: readValue.text,
    value,
    quoted: true,
...
function tokenizeText(input, keyValueTok, tempState, depth = 0) {
  ...
  while (true) {
    ...
    tokens.push(...toks);
    if (tokens[tokens.length - 1].type === exports.TextTokenType.EOF) break;
  }
  // increment depth
  depth++;
  // tokenize expression inside EXPR tokens
  for (const t of tokens)
    if (t.type === exports.TextTokenType.EXPR)
...
function nextTextToken(state, tempState, parentTok, depth) {
  ...
  if (eof(state)) {
    ...
    linePos = getLinePosFromPos(tempState.lineStarts, pos);
    eofToken = {
      type: exports.TextTokenType.EOF,
      raw: "",
      text: "",
      value: "",
      quoted: false,
  ...
  if (peek(state, 2) === "${") {
    ...
    linePos = getLinePosFromPos(tempState.lineStarts, pos);
    exprToken = {
      type: exports.TextTokenType.EXPR,
      raw: readValue.raw,
      text: readValue.text,
      value,
      quoted: false,
  ...
  linePos = getLinePosFromPos(tempState.lineStarts, pos);
  textToken = {
    type: exports.TextTokenType.TEXT,
    raw: readValue.raw,
    text: readValue.text,
    value,
    quoted: false,
...
async function handleStrPath(node, pathPart, state, tempState) {
  ...
  if (node instanceof yaml.YAMLMap) {
    for (const pair of node.items) {
      let key;
      if (pair.key instanceof yaml.Scalar) key = pair.key.value;
      else key = pair.key;
      if (key === pathPart) {
  ...
  if (
    isRecord(node) &&
    !(node instanceof yaml.Scalar) &&
    !(node instanceof yaml.Alias)
  )
...
async function handleThis(ctx, state, tempState) {
  ...
  const args =
    (_b = (_a = ctx.args) === null || _a === void 0 ? void 0 : _a.argsObj) !==
      null && _b !== void 0
      ? _b
      : {};
...
function handleParam(ctx, state, tempState) {
  ...
  const defParam =
    param === null || param === void 0 ? void 0 : param.defauleValue;
  // if value is passed for this alias use it otherwise use default value
  const value =
    (_e =
      (_d =
        (_b =
          (_a = tempState.options.params) === null || _a === void 0
            ? void 0
            : _a[alias]) !== null && _b !== void 0
          ? _b
          : (_c = tempState.options.universalParams) === null || _c === void 0
          ? void 0
          : _c[alias]) !== null && _d !== void 0
        ? _d
        : defParam) !== null && _e !== void 0
      ? _e
      : null;
...
function handleLocal(ctx, state, tempState) {
  ...
  let defLocal =
    local === null || local === void 0 ? void 0 : local.defauleValue;
  ...
  const value =
    (_b =
      (_a = handledLocalsVal[alias]) !== null && _a !== void 0
        ? _a
        : defLocal) !== null && _b !== void 0
      ? _b
      : null;
...
async function handleTextTokens(tokens, state, tempState) {
  ...
  while (i < tokens.length) {
    const tok = tokens[i];
    if (tok.type === exports.TextTokenType.TEXT) {
      out += tok.text;
      tok.resolvedValue = tok.text;
    }
    if (tok.type === exports.TextTokenType.EXPR) {
      const value = await handleExprTokens(
        tok,
        tok.exprTokens,
        state,
...
async function handleExprTokens(textToken, tokens, state, tempState) {
  ...
  for (const tok of tokens) {
    switch (tok.type) {
      case exports.ExprTokenType.BASE:
        ctx.base = { value: tok.text, tok };
        break;
      case exports.ExprTokenType.PATH:
        ...
        break;
      case exports.ExprTokenType.DOT:
...
async function handleArgTokens(tokens, state, tempState) {
  ...
  for (const tok of tokens) {
    ...
    if (tok.type === exports.ArgsTokenType.COMMA) {
    ...
    if (tok.type === exports.ArgsTokenType.KEY_VALUE) {
...
async function handleKeyValueTokens(tokens, state, tempState) {
  ...
  for (const tok of tokens) {
    ...
    if (tok.type === exports.KeyValueTokenType.KEY) {
    ...
    if (tok.type === exports.KeyValueTokenType.VALUE) {
...
async function resolveUnknown(item, anchored, state, tempState) {
  if (item instanceof yaml.Alias) return resolveAlias(item, tempState);
  ...
  deleteDep(path) {
    ...
    if (graph)
      for (const p of graph)
        (_a = this.reverseDepGraphs.get(p)) === null || _a === void 0
          ? void 0
          : _a.delete(path);
    if (reverseGraph)
      for (const p of reverseGraph)
        (_b = this.depGraphs.get(p)) === null || _b === void 0
          ? void 0
          : _b.delete(path);
...
async function parseExtend(path, options = {}, source, state) {
  ...
  try {
    ...
    const comParams = {
      ...
      ...((_b = ts.options.universalParams) !== null && _b !== void 0
        ? _b
        : {}),
    };
    ...
    const err = new YAMLExprError(
      [0, 0],
      "",
      `Unkown error thrown: ${error.message}`
    );
    ts.errors.push(err);
    return {
      parse: undefined,
 ...
 */
function initTempState(path$1, options) {
  var _a;
  const basePath =
    (_a =
      options === null || options === void 0 ? void 0 : options.basePath) !==
      null && _a !== void 0
      ? _a
      : process.cwd();
  return {
    ...
    errors: [],
    importedErrors: [],
    resolvedPath: path.resolve(basePath, path$1),
    filename: "",
    pos: [0, 0],
    anchors: new Map(),
    locals: [],
