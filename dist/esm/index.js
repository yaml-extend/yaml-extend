import { Type as Type$1, Schema as Schema$1, JSON_SCHEMA as JSON_SCHEMA$1, FAILSAFE_SCHEMA as FAILSAFE_SCHEMA$1, DEFAULT_SCHEMA as DEFAULT_SCHEMA$1, CORE_SCHEMA as CORE_SCHEMA$1, load as load$1, YAMLException, dump as dump$1 } from 'js-yaml';
export { YAMLException } from 'js-yaml';
import { readFileSync, realpathSync, watch, writeFileSync } from 'fs';
...
function readFile(resPath, currentPath) {
    ...
    return readFileSync(resPath, { encoding: "utf8" });
}
/**
 * Function to resolve paths by adding basepath (path of the current module) and path (path of the imported or read module) together making absolute path of them.
...
async function readFileAsync(resPath, currentPath) {
    ...
    return await readFile$1(resPath, { encoding: "utf8" });
}
/**
...
class ImportHandler {
    ...
    async importAsync(modulePath, targetPath, targetParams, loadOpts, loadId) {
        ...
        const resPath = __classPrivateFieldGet(this, _ImportHandler_instances, "m", _ImportHandler_handlePath).call(this, (_a = loadOpts === null || loadOpts === void 0 ? void 0 : loadOpts.basePath) !== null && _a !== void 0 ? _a : process.cwd(), dirModulePath, targetPath, loadId);
        // read YAML file and get string
        const str = await readFile$1(resPath, { encoding: "utf8" });
        // load str
...
_ImportHandler_load = new WeakMap(), _ImportHandler_loadAsync = new WeakMap(), _ImportHandler_instances = new WeakSet(), _ImportHandler_handlePath = function _ImportHandler_handlePath(basePath, modulePath, targetPath, loadId) {
    ...
    // return path
    return resPath;
}, _ImportHandler_removeFileName = function _ImportHandler_removeFileName(path) {
...
function addModuleCache(loadId, str, filepath, blueprint, dirObj) {
    // resolve filepath
    const resPath = resolve$1(filepath);
...
function load(str, opts) {
    ...
    if (str === undefined)
        throw new YAMLException(`You should pass either YAML string or url path of YAML file in str.`);
    // set new loadId
...
async function resolveAsync(str, opts) {
    ...
    if (opts === null || opts === void 0 ? void 0 : opts.outputPath) {
        ...
        if (!isYaml)
            throw new WrapperYAMLException(`Target path supplied to resolve function is not YALM file.`);
        writeFile(resPath, dumped, { encoding: "utf8" });
    }
    // return dumped value
    return dumped;
}
